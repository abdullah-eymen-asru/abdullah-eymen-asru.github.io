...
/**
 * `govdeEl` içine render edilmiş HTML'i TARAR (h2/h3 başlıklarını bulur),
 * her birine bir `id` atar (yoksa) ve _layouts/post.html'deki
...
export function tocOlustur(govdeEl, tocIstendi) {
  ...
  const basliklar = govdeEl.querySelectorAll("h2, h3");
  ...
  basliklar.forEach((baslik) => {
    ...
    const girintiSinifi = baslik.tagName === "H3" ? " toc-alt-baslik" : "";
    satirlar.push(
